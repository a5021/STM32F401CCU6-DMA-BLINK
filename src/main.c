...
static const unsigned led_data[30] = {            /* data to send to GPIOC->BSTR once TIM1 update event occures   */
...
__STATIC_INLINE void init(void) {
  ...
  RCC->AHB1ENR = (                 /* 0x40023830: RCC AHB1 peripheral clock register                              */
  ...
  RCC->APB2ENR =                   /* 0x40023844: RCC APB2 peripheral clock enable register                       */
    RCC_APB2ENR_TIM1EN;            /* (1 << 0)  Enable clock for TIM1 peripheral                     0x00000000   */

  GPIOC->MODER =                   /* 0x40020800: GPIOC port mode register                                        */
  ...
  DMA2_Stream5->CR = (             /* 0x40026488: DMA stream 5 configuration register                             */
    ...
    DMA_SxCR_CHSEL_2       |       /* 0x08000000    0x06 -- 6th channel                                           */
    ...
    DMA_SxCR_MINC          |       /* (1 << 10)     Increment memory address mode                    0x00000400   */
  ...
  TIM1->PSC  = 100 - 1;            /* 0x40010028: TIM prescaler;                     Config input clock divider   */
  TIM1->EGR  = TIM_EGR_UG;         /* 0x40010014: TIM event generation register;     Force Update Event           */
  TIM1->ARR  = 33333 - 1;          /* 0x4001002C: TIM auto-reload register;          Set period                   */
  TIM1->DIER = TIM_DIER_UDE;       /* 0x4001000C: TIM DMA/interrupt enable register; Enable DMA request on update */
  TIM1->CR1  = TIM_CR1_CEN;        /* 0x40010000: TIM control register 1;            Enable counter               */
